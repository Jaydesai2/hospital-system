        ...
        <div class="bg-white rounded-[3rem] shadow-2xl shadow-slate-200/50 border border-slate-50 overflow-hidden">
            ...
            <div class="p-12 grid grid-cols-2 gap-12 border-b border-slate-50">
                <div class="space-y-4">
                    ...
                    <div class="space-y-1">
                        <p class="font-black text-slate-900 text-base tracking-tight">{{ $invoice->patient->user->name ?? 'Unknown' }}</p>
                        <p class="text-sm font-medium text-slate-500 italic">{{ $invoice->patient->user->email ?? 'N/A' }}</p>
                    </div>
                </div>
            ...
            <!-- Footer -->
            <div class="p-12 bg-slate-50/50 flex flex-col md:flex-row justify-between items-end gap-8">
                <div class="space-y-6">
                    <div class="space-y-2">
                    ...
                    <div class="space-y-3 pt-6 border-t border-slate-200">
                        <div class="flex justify-between gap-12">
                            <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Consultation Fee</span>
                            <span class="text-sm font-bold text-slate-600">${{ number_format($invoice->consultation_fee, 2) }}</span>
                        </div>
                        <div class="flex justify-between gap-12">
                            <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Prescription Charges</span>
                            <span class="text-sm font-bold text-slate-600">${{ number_format($invoice->prescription_fee, 2) }}</span>
                        </div>
                        @if($invoice->additional_charges > 0)
                        <div class="flex justify-between gap-12">
                            <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Additional Charges</span>
                            <span class="text-sm font-bold text-slate-600">${{ number_format($invoice->additional_charges, 2) }}</span>
                        </div>
                        @endif
                    </div>
                </div>
